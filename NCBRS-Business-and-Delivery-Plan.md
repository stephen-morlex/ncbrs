...
| 11e | **Side finding, not a test artifact: a common given name can carry two different children over the duplicate threshold.** Real names "Nyandeng Deng" and "Nyandeng Garang", same day, same sex, same facility, score 35 + 10 + 18 = 63 ≥ 60. The whole-string Levenshtein lets a long shared given name dominate. At a busy hospital, with a given name that common, that is a steady stream of false duplicates in the review queue | **Data-free part done** — the problem was broader than given names: whole-string edit distance rewards *any* shared word, and every one of the demo seed's five "duplicates" was two different people sharing one word (mothers "Achol Deng"/"Aluel Deng" scored 70%; one pair 94). Names are now aligned word by word, in any order; one aligned pair of real words under 50% alike means different names, while a misspelt word, a missing word or a swapped order still matches (word order had never matched before). The seed now yields exactly one candidate, a duplicate planted on purpose. **Still open, needs data:** weighting a shared rare word above a shared common one |
