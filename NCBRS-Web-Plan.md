...
- [ ] Accessibility pass to WCAG 2.2 AA — this is a government service. *Structural gaps closed and gated:* skip link (2.4.1 Bypass Blocks), per-route `document.title` (2.4.2 Page Titled) and focus moved to main on route change — all three are the classic single-page-app failures, and none of them is visible to a sighted mouse user, which is why they survived this long. `jsx-a11y` now runs in CI (`--deny-warnings`), so a regression fails the build. *Automated WCAG 2.2 AA scan in CI:* `e2e/accessibility.spec.ts` runs axe-core over all 20 pages, signed in as a role that can see each one, at desktop and phone width. That covers contrast, 2.5.8 target size, names, roles and landmarks. Its first run found and led to fixes for two theme tokens under 4.5:1 (`--muted-foreground`, `--destructive`), scrolling tables unreachable by keyboard on a phone, and a list without list items. **Still outstanding for a conformance claim:** testing with real assistive technology, which cannot be asserted by a rule engine.
